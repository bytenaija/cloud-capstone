...
To implement this project, you need to implement a simple URL application using AWS Lambda and Serverless framework. Search for all comments starting with the `URL:` in the code to find the placeholders that you need to implement.
...
* `name` (string) - name of a URL item (e.g. "Change a light bulb")
...
To store URL items, you might want to use a DynamoDB table with local secondary index(es). A create a local secondary index you need to create a DynamoDB resource like this:
...
UrlsTable:
  ...
  Properties:
    ...
    LocalSecondaryIndexes:
      - IndexName: ${self:provider.environment.URL_INDEX}
        KeySchema:
          - AttributeName: partitionKey
            KeyType: HASH
          - AttributeName: indexKey
...
This should start a development server with the React application that will interact with the serverless URL application.
